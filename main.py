...
def train_one_dataset(params, file_name, train_q_data, train_qa_data, valid_q_data, valid_qa_data):
    ...
    lr_scheduler = mx.lr_scheduler.FactorScheduler(
        step=20*(train_q_data.shape[0]/params.batch_size), factor=0.667, stop_factor_lr=1e-5)

    net.init_optimizer(optimizer='sgd', optimizer_params={
                       'learning_rate': params.lr, 'momentum': params.momentum, 'lr_scheduler': lr_scheduler})
    ...
    for idx in range(params.max_iter):
        train_loss, train_accuracy, train_auc = train(
            net, params, train_q_data, train_qa_data, label='Train')
        valid_loss, valid_accuracy, valid_auc = test(
            net, params, valid_q_data, valid_qa_data, label='Valid')
...
def test_one_dataset(params, file_name, test_q_data, test_qa_data):
    ...
    test_net.bind(data_shapes=[
        mx.io.DataDesc(name='q_data', shape=(
            params.seqlen, params.batch_size), layout='SN'),
        mx.io.DataDesc(name='qa_data', shape=(params.seqlen, params.batch_size), layout='SN')],
        label_shapes=[mx.io.DataDesc(name='target', shape=(params.seqlen, params.batch_size), layout='SN')])
    ...
    test_loss, test_accuracy, test_auc = test(
        test_net, params, test_q_data, test_qa_data, label='Test')
    log_info = "\ntest_auc:\t{}\ntest_accuracy:\t{}\ntest_loss:\t{}\n".format(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seedNum', type=int,
                        default=1024, help='the random seed')

    # synthetic / assist2009_updated / assist2015 / KDDal0506 / STATICS
    ...
    if dataset == "synthetic":
        ...
        parser.add_argument('--load', type=str,
                            default='synthetic/v1', help='model file to load')
        parser.add_argument('--save', type=str,
                            default='synthetic/v1', help='path to save model')
    if dataset == "assist2009_updated":
        ...
        parser.add_argument('--batch_size', type=int,
                            default=50, help='the batch size')
        parser.add_argument('--q_embed_dim', type=int,
                            default=50, help='question embedding dimensions')
        ...
        parser.add_argument('--load', type=str,
                            default='assist2015', help='model file to load')
        parser.add_argument('--save', type=str,
                            default='assist2015', help='path to save model')
